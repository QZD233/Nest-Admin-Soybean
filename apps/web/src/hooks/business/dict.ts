...
import { fetchDictFindDataByType } from '@/service/api';
...
export function useDict(dictType: string, immediate: boolean = true) {
  ...
  async function getData() {
    ...
    if (dicts) {
      data.value = dicts as Api.System.DictData[];
      return;
    }
    try {
      const { data: dictData } = await fetchDictFindDataByType(dictType);
      if (!dictData) {
        return;
      }
      const dictDataList = dictData as Api.System.DictData[];
