...
import {
  fetchLogininforFindAll,
  fetchLogininforRemove,
  fetchLogininforRemoveAll,
  fetchLogininforUnlock
} from '@/service/api';
...
const {
  ...
  apiFn: fetchLogininforFindAll as any,
  apiParams: {
    pageNum: 1,
    pageSize: 10,
  ...
  columns: (() => [
    ...
    },
    {
      ...
      align: 'center',
      minWidth: 120,
      render: (row: Api.Monitor.LoginInfor) => {
...
async function handleBatchDelete() {
  // request
  try {
    await fetchLogininforRemove(checkedRowKeys.value.join(','));
    onBatchDeleted();
  } catch {
...
async function handleCleanLoginInfor() {
  window.$dialog?.error({
    ...
    onPositiveClick: async () => {
      try {
        await fetchLogininforRemoveAll();
...
async function handleUnlockLoginInfor(username: string) {
  try {
    await fetchLogininforUnlock(username);
  ...
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard title="登录日志列表" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
      ...
      <NDataTable
        ...
        class="sm:h-full"
      />
      <LoginInforViewDrawer v-model:visible="drawerVisible" :row-data="editingData as Api.Monitor.LoginInfor" />
    </NCard>
  </div>
</template>
